...
void NetworkManager::loadWebPage(){
    ...
    QString urlString = QString("https://www.alphavantage.co/query?function=DIGITAL_CURRENCY_DAILY&symbol=BTC&market=RUB&apikey=%0").arg(apikey);
...
void NetworkManager::replyFinished(QNetworkReply *reply)
{
    ...
    QList<QPair<QString,QString>> graphValuesRUB;
    ...
    if(doc.isArray()==true){
        ...
        for (QString k :keys){
            ...
            QString hufValue = dayValues["1a. open (RUB)"].toString();
            ...
            graphValuesRUB.append(dataItem);
    ...
    for (int i=0; i<graphValuesRUB.size(); i++){
        QPair<QString,QString> data = graphValuesRUB[i];
